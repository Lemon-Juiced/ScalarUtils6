...
public class UmbraliteShovelItem extends ShovelItem {
    ...
    public boolean mineBlock(ItemStack stack, Level level, BlockState state, BlockPos pos, LivingEntity miningEntity) {
        if (!level.isClientSide && !state.isAir()) {
            int breaks = stack.get(ScalarDataComponents.UMBRALITE_SHOVEL_BREAKS.get()) != null ? stack.get(ScalarDataComponents.UMBRALITE_SHOVEL_BREAKS.get()) : 0;
            stack.set(ScalarDataComponents.UMBRALITE_SHOVEL_BREAKS.get(), breaks + 1);
        }
    ...
    public void appendHoverText(ItemStack stack, TooltipContext context, List<Component> tooltipComponents, TooltipFlag tooltipFlag) {
        ...
        if (hasteLevel > 0) {
            tooltipComponents.add(Component.translatable("tooltip.scalarutils.umbralite_shovel.haste", hasteLevel).withStyle(ChatFormatting.AQUA));
        } else {
            tooltipComponents.add(Component.translatable("tooltip.scalarutils.umbralite_shovel.no_haste").withStyle(ChatFormatting.GRAY));
        }
        if (nextLevel != -1) {
            tooltipComponents.add(Component.translatable("tooltip.scalarutils.umbralite_shovel.breaks_to_next", nextLevel - breaks).withStyle(ChatFormatting.YELLOW));
        } else {
            tooltipComponents.add(Component.translatable("tooltip.scalarutils.umbralite_tool.max_level").withStyle(ChatFormatting.GREEN));
        }
        tooltipComponents.add(Component.translatable("tooltip.scalarutils.umbralite_shovel.total_breaks", breaks).withStyle(ChatFormatting.DARK_PURPLE));
    }
}