...
public class UmbralitePickaxeItem extends PickaxeItem {
    ...
    public boolean mineBlock(ItemStack stack, Level level, BlockState state, BlockPos pos, LivingEntity miningEntity) {
        if (!level.isClientSide && !state.isAir()) {
            int breaks = stack.get(ScalarDataComponents.UMBRALITE_PICKAXE_BREAKS.get()) != null ? stack.get(ScalarDataComponents.UMBRALITE_PICKAXE_BREAKS.get()) : 0;
            stack.set(ScalarDataComponents.UMBRALITE_PICKAXE_BREAKS.get(), breaks + 1);
        }
    ...
    public void appendHoverText(ItemStack stack, TooltipContext context, List<Component> tooltipComponents, TooltipFlag tooltipFlag) {
        ...
        if (hasteLevel > 0) {
            tooltipComponents.add(Component.translatable("tooltip.scalarutils.umbralite_pickaxe.haste", hasteLevel).withStyle(ChatFormatting.AQUA));
        } else {
            tooltipComponents.add(Component.translatable("tooltip.scalarutils.umbralite_pickaxe.no_haste").withStyle(ChatFormatting.GRAY));
        }
        if (nextLevel != -1) {
            tooltipComponents.add(Component.translatable("tooltip.scalarutils.umbralite_pickaxe.breaks_to_next", nextLevel - breaks).withStyle(ChatFormatting.YELLOW));
        } else {
            tooltipComponents.add(Component.translatable("tooltip.scalarutils.umbralite_tool.max_level").withStyle(ChatFormatting.GREEN));
        }
        tooltipComponents.add(Component.translatable("tooltip.scalarutils.umbralite_pickaxe.total_breaks", breaks).withStyle(ChatFormatting.DARK_PURPLE));
    }
}